  ...
  return (
    <div className={`${dim} rounded-2xl flex items-center justify-center overflow-hidden shrink-0 shadow-lg ${iconSrc ? '' : color}`}>
      {iconSrc
        ...
        : <span className="text-white font-bold text-lg" style={{ fontFamily: 'var(--font-outfit)' }}>{letter}</span>}
    </div>
  );
};
  ...
  return (
    <div className="mb-8 p-6 rounded-[2rem] bg-gradient-to-br from-orange-500/10 to-transparent border border-orange-500/20">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-bold flex items-center gap-2 mb-2 text-white" style={{ fontFamily: 'var(--font-outfit)' }}>
            <Award size={18} className="text-orange-400" /> Perfect Tracking
          </h3>
          <p className="text-4xl font-black text-orange-400 text-glow" style={{ fontFamily: 'var(--font-outfit)' }}>{days} day{days !== 1 ? 's' : ''}</p>
...
const WeeklyTrend = ({ data }: { data: WeeklyStatDay[] }) => {
  ...
  return (
    <div className="mb-8 p-6 rounded-[2rem] glass-card">
      <h3 className="font-bold mb-6 flex items-center gap-2 text-white" style={{ fontFamily: 'var(--font-outfit)' }}>
...
export default function App() {
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}>
          <Loader size={32} className="text-white opacity-50" />
        </motion.div>
      </div>
    );
  }
  ...
  const OnboardingView = () => {
    const steps = [
      { title: 'Extreme Focus.', description: 'The absolute way to reclaim your time. Set hard limits, and if you break them, it cuts you off or costs you.', icon: <Smartphone className="text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.8)]" size={48} /> },
      { title: 'Usage Tracking', description: 'Grant access so FocusFine can precisely measure your screen time.', icon: <BarChart3 className="text-emerald-400 drop-shadow-[0_0_15px_rgba(52,211,153,0.8)]" size={48} />, permission: 'Enable Tracking', action: () => window.Android?.requestUsageAccess() },
      { title: 'The Lock Screen', description: 'Allow us to overlay the lock screen over distractive apps instantly.', icon: <ShieldCheck className="text-blue-400 drop-shadow-[0_0_15px_rgba(96,165,250,0.8)]" size={48} />, permission: 'Enable Lock Screen', action: () => window.Android?.requestOverlay() },
      { title: 'Unstoppable Monitor', description: 'To prevent you from bypassing limits, FocusFine needs to run persistently in the background.', icon: <Zap className="text-amber-400 drop-shadow-[0_0_15px_rgba(251,191,36,0.8)]" size={48} />, permission: 'Enable Unstoppable Mode', action: () => window.Android?.requestBatteryOptimization() },
    ];
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[90vh] text-center px-6 relative overflow-hidden">
        ...
        <motion.div key={onboardingStep} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, y: -20 }} className="max-w-md w-full relative z-10">
          ...
          <h1 className="text-4xl font-black tracking-tight mb-4 text-white" style={{ fontFamily: 'var(--font-outfit)' }}>{current.title}</h1>
          <p className="text-zinc-400 text-lg mb-12 leading-relaxed">{current.description}</p>
          <div className="space-y-4">
            {current.permission ? (
              <Button onClick={handlePermissionTap} className="w-full py-5 text-lg shadow-[0_0_30px_rgba(255,255,255,0.15)]">{current.permission}</Button>
            ) : (
              <Button onClick={() => setOnboardingStep(1)} className="w-full py-5 text-lg shadow-[0_0_30px_rgba(255,255,255,0.15)]">Begin Setup <ArrowRight size={20} /></Button>
            )}
    ...
    return (
      <div className="max-w-2xl mx-auto px-6 py-16 relative">
        <header className="mb-10 text-center relative z-10">
          <p className="text-emerald-400 text-xs font-bold uppercase tracking-widest mb-2" style={{ fontFamily: 'var(--font-outfit)' }}>Phase 1</p>
          <h1 className="text-4xl font-black mb-3 text-white" style={{ fontFamily: 'var(--font-outfit)' }}>Select Targets</h1>
        ...
        {loadingApps ? (
          ...
          <div className="space-y-4 mb-12 max-h-[55vh] overflow-y-auto pb-6 custom-scrollbar relative z-10">
            {installedApps.map((app, i) => (
              <motion.div 
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-white text-lg" style={{ fontFamily: 'var(--font-outfit)' }}>{app.appName}</h3>
                    {app.isSelected && (
        ...
        <div className="fixed bottom-0 left-0 w-full p-6 bg-gradient-to-t from-black via-black to-transparent z-20">
          <Button disabled={selectedCount === 0 || loadingApps} onClick={finishSetup} className="w-full py-5 text-lg shadow-[0_0_30px_rgba(255,255,255,0.1)]">
    ...
    return (
      <div className="max-w-2xl mx-auto px-6 py-12 pb-32 relative">
        ...
        <header className="flex items-center justify-between mb-10 relative z-10">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-white flex items-center gap-3" style={{ fontFamily: 'var(--font-outfit)' }}>
              FocusFine <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,1)]" />
            </h1>
        ...
        <div className="mb-8 p-10 rounded-[2.5rem] glass-card relative overflow-hidden z-10 border border-white/10 group">
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <div className="relative">
              ...
              <h2 className="text-8xl font-black text-white text-glow tracking-tighter" style={{ fontFamily: 'var(--font-outfit)' }}>
                {stats.focusScore}
              </h2>
            </div>
            ...
            <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden mt-8 shadow-inner">
        ...
        <div className="grid grid-cols-2 gap-4 mb-10 relative z-10">
          <Card className="flex flex-col items-center text-center hover:bg-zinc-800/40 transition-colors">
            ...
            <h2 className="text-3xl font-bold text-white" style={{ fontFamily: 'var(--font-outfit)' }}>${stats.totalSpentToday.toFixed(2)}</h2>
          ...
          <Card className="flex flex-col items-center text-center hover:bg-zinc-800/40 transition-colors">
            ...
            <h2 className="text-3xl font-bold text-emerald-400 text-glow" style={{ fontFamily: 'var(--font-outfit)' }}>{timeSavedHrs}<span className="text-sm font-semibold text-zinc-500 ml-1 uppercase">hrs</span></h2>
          </Card>
        </div>
        ...
        <div className={`relative z-10 mb-10 p-6 rounded-[2rem] transition-all flex items-center justify-between ${stats.strictMode ? 'bg-red-950/30 border border-red-500/30 shadow-[0_0_30px_rgba(239,68,68,0.15)]' : 'glass-card'}`}>
          <div className="flex items-center gap-5">
            ...
            <div>
              <h3 className={`font-bold text-lg mb-0.5 ${stats.strictMode ? 'text-red-400' : 'text-white'}`} style={{ fontFamily: 'var(--font-outfit)' }}>Hardcore Mode</h3>
        ...
        <div className="relative z-10">
          <h2 className="text-xl font-bold mb-6 flex items-center gap-3 text-white" style={{ fontFamily: 'var(--font-outfit)' }}>
            Target Locks
          ...
          <div className="space-y-4">
            {monitoredApps.length === 0 && !loadingDashboard && (
              <div className="glass-card p-8 rounded-[2rem] text-center">
                ...
                <Button onClick={() => setStep('setup')} variant="outline" className="w-full">Deploy First Lock</Button>
              </div>
            )}
            <AnimatePresence>
                  ...
                  <motion.div key={app.packageName} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, height: 0 }}
                    ...
                    <div className="flex items-center justify-between relative z-10">
                      <div className="flex items-center gap-5">
                        <AppIcon packageName={app.packageName} appName={app.appName} />
                        <div>
                          <h3 className="font-bold text-white text-lg" style={{ fontFamily: 'var(--font-outfit)' }}>{app.appName}</h3>
                    ...
                    <div className="absolute bottom-0 left-0 w-full h-1.5 bg-zinc-800">
                      <div className={`h-full transition-all duration-700 ${isOver ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)]' : 'bg-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.5)]'}`}
                        style={{ width: `${pct}%` }} />
                    </div>
                  </motion.div>
                );
